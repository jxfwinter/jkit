...
public:
    ...
    Http2ConnectionPtr get_http2s_connect(const HttpsReqArgument &args) noexcept
    {
        ...
        session.on_connect([&promise](boost::asio::ip::tcp::resolver::iterator endpoint) {
            promise.set_value();
        });
        session.on_error([conn_ptr, this](const boost::system::error_code &error) {
            LogErrorExt << ec.message();
...
StrResponse MultiClientHttp::h2_req(const StrRequest &req, const HttpsReqArgument& args) noexcept
{
    ...
    const nghttp2::asio_http2::client::request *request = session.submit(ec,
                                                                         req.method_string(),
                                                                         "https://" + conn.host + ":" + conn.port + req.target(),
                                                                         body,
                                                                         h2_headers);
    if(ec)
    {
    ...
    request->on_response([&res, &promise, this]( const nghttp2::asio_http2::client::response &response ) {
        res.result(static_cast<unsigned>(response.status_code()));
        for (auto &kv : response.header())
        {
            res[kv.first] = kv.second.value;
        }
        if(response.content_length() == 0 || response.content_length() == -1)
        {
            promise.set_value();
        }
        else
        {
            response.on_data([&res, &promise](const uint8_t *data, std::size_t len ) {
                if(len > 0)
                {
                    res.body() = std::string((const char*)data, len);
                }
